package com.spring.cloud.employeeclient.service



import com.spring.cloud.employeeclient.feign.client.RobotFeignClient
...
class RobotClientService(internal val robotClient: RobotFeignClient) {
    ...
    fun createRobots() {
        robotNames.forEach { robotName: String ->
            robotClient.addRobot(RobotModel(robotName+  System.currentTimeMillis(), "HOVER"))
        }
    }
    ...
    fun addConstructionPlan(robotName: String) {
        require(robotConstructionPlans.containsKey(robotName)) {
            "No construction plan exists for robot $robotName"
        }
